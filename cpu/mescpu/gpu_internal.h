...
/// The Queue holds information of the operation that is beeing currently
/// transmitted.
struct Queue {
    Operation operation;
...
/// @note When all parameters are 0 the GPU expects the whole video buffer.
/// 160*120@3bpp (7200 bytes)
/// @attention When sending the whole buffer, use 0 for all parameters. This
/// operation is much faster.
/// @related See `display_buf` if you want to display the buffer after sending
/// it.
/// @return 00 00 BF 00 XX YY SX SY
Operation gpu_operation_send_buf(uint8_t bf, uint8_t xx, uint8_t yy, uint8_t sx,
                                 uint8_t sy);
...
Operation gpu_operation_print_text(uint8_t bf, uint8_t foreground,
                                   uint8_t background, uint8_t size, uint8_t ox,
                                   uint8_t oy);
...
/// @brief Continue execution of the programm and send data to the GPU in the
/// background.
/// @details Will set `spi_dma_transmit_ongoing` to true while the transmission
/// is ongoing,
