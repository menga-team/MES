...
uint8_t __attribute__((section(".buffer_b")))
buffer_b[(BUFFER_WIDTH * BUFFER_HEIGHT * BUFFER_BPP) /
         (CHAR_BIT * sizeof(uint8_t))];
...
uint8_t gpu_get_pixel(const void *buffer, uint16_t position) {
    ...
    uint32_t bytes =
        (*(uint32_t *)(buffer + (position / 8) * BUFFER_BPP)) /*& 0x00FFFFFF*/;
...
void gpu_write(void *buffer, uint8_t x, uint8_t y, uint8_t fg, uint8_t bg,
    ...
    while (text[i] != '\0') {
        ...
        for (int k = 0; k < 8; ++k) {
            // Reading the code for `gpu_set_pixel` might help with
            // understanding this one.
            uint16_t position = ((y + k) * BUFFER_WIDTH + x);
            uint32_t *bytes =
                (uint32_t *)(buffer + (position / 8) * BUFFER_BPP);
            // This last shift might not be accurate, but it does not matter
            // because it will get corrected. We just need a value that is
            // garanteed not to be 0. If it is 0, we would be wasting cpu time
            // because of the if in the loop below.
            uint8_t last_shift = -1;
            for (uint8_t j = 0; j < 6; ++j) {
                uint8_t shift = (7 - ((position + j) % 8)) * BUFFER_BPP;
                if (last_shift == 0) {
                    // 0 is the last possible shift, meaning we reached the end.
                    // We need to update our bytes because the next pixel won't
                    // fit. Shift of 21 is the first pixel & shift of 0 is the
                    // last pixel.
                    bytes = (uint32_t *)(buffer +
                                         ((position + j) / 8) * BUFFER_BPP);
                }
                *bytes =
                    (*bytes & ~(PIXEL_MASK << shift)) |
                    ((((console_font_6x8[8 * to_print + k] << j) & (1 << 7))
                          ? fg
                          : bg)
                     << shift);
                last_shift = shift;
        ...
        x += 6;
        if (x > 154) { // character wont fit anymore
            x = 0;
            y += 8;
        }
        i++;
